...
def main() -> None:
    ...
    if use_duckdb:
        ...
        _show_duckdb_dashboard(db_path)
        return
...
def _show_duckdb_dashboard(db_path: str) -> None:
    tab_labels = [
        "Monthly Overview",
        "Top-N Aspects",
        "Aspect Detail",
...
def _show_priority_confidence_scatter(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return
    chart = (
        alt.Chart(alt.Data(values=rows))
        ...
        .encode(
            ...
            y=alt.Y("Priority:Q", title="Priority score"),
            size=alt.Size(
                "Severity:Q",
                title="Severity",
...
def _show_peer_benchmark_charts(rows: list[dict[str, Any]]) -> None:
    ...
    gap_chart = (
        alt.Chart(alt.Data(values=normalized_rows))
        .mark_bar()
        .encode(
            x=alt.X("Peer gap:Q", title="Peer gap"),
            y=alt.Y("Aspect:N", sort="-x", title="Aspect"),
            color=alt.condition(
                alt.datum["Peer gap"] >= 0,
                alt.value("#d62728"),
                alt.value("#2ca02c"),
            ),
            tooltip=["Aspect:N", "Peer gap:Q", "Target negative rate:Q", "Peer avg:Q", "Peer restaurants:Q"],
        )
        .properties(height=max(240, 32 * len(normalized_rows)))
    )
    zero_rule = alt.Chart(alt.Data(values=[{"x": 0}])).mark_rule(color="gray").encode(x="x:Q")
    st.caption("Peer gap by aspect")
    st.altair_chart(gap_chart + zero_rule, width="stretch")
